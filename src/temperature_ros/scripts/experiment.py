...
class Config():
    ...
    control_points_path = f'src/temperature_ros/config/exp.yaml'
    ...
    while not rospy.is_shutdown():   
        ...
        if status == Status.IDLE:
            ...
            if (object_msg is None) or (len(object_msg.detections)<=0):
                print(f"\r[INFO] [ ] Bottle Detected ", end='', flush=True)
                continue
            else:
                ...
                if len(object_msg.detections)>0:
                    if utils.check_detections(object_msg.detections, classes, ['bottle']):
                        print(f"\r[INFO] [✔️] Bottle Detected ", end='', flush=True)
                        if config.auto:
                            status = Status.APPROACH
                            transition = True
                    elif utils.check_detections(object_msg.detections, classes, ['cup']):
